...
const CREATE_MOVIE = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: "Movie is created", response: response });
  } catch (err) {
...
const GET_ALL_MOVIES = async (req, res) => {
  try {
    const movies = await movieSchema.find(/*{ id: req.body.id }*/);
...
const DELETE_MOVIE = async (req, res) => {
  try {
    ...
    await movieSchema.findOneAndDelete({ id: movieId });
...
const GET_MOVIE_BY_ID = async (req, res) => {
  try {
    ...
    if (!movie) {
      return res
        .status(404)
        .json({ message: "There are no movie with such id" });
    }
